...
.scene-num { color: #ff6b35; font-weight: 700; font-size: 0.8rem; text-transform: uppercase; letter-spacing: 0.1em; }
...
.stat-box { background: #13131f; border: 1px solid #2a2a3f; border-radius: 8px; padding: 0.8rem 1rem; text-align: center; }
...
st.markdown('<div class="hero-sub">역사 + 경제 연결 콘텐츠 · 1분 쇼츠 자동 생성 · Powered by Claude</div>', unsafe_allow_html=True)

# API 키 체크
api_key = get_api_key()
if not api_key:
    st.error("⚠️ ANTHROPIC_API_KEY가 설정되지 않았습니다. `.streamlit/secrets.toml` 또는 Streamlit Cloud Secrets에 API 키를 입력해주세요.")
    st.stop()
...
{{
  ...
  "scenes": [
    {{
      ...
      "image_keyword_en": "image search keyword(English for Pexels)"
    }}
  ],
  "full_narration": "전체 나레이션을 하나의 흐름으로 (자연스럽게)",
  "hashtags": ["태그1","태그2","태그3","태그4","태그5","태그6","태그7","태그8","태그9","태그10"],
  "production_tips": "이 영상 제작 시 주의할 점 또는 추천 배경음악 분위기"
}}"""

    with st.spinner("🎬 Claude가 콘텐츠를 생성하는 중..."):
        try:
...
<div class="scene-card">
  ...
  <div class="scene-img">{img_line}</div>
            ...
            if gen_narration and "full_narration" in data:
                ...
                st.caption("👆 복사 후 [타입캐스트](https://typecast.ai) 에 붙여넣으세요")
...
# 푸터
with st.expander("📖 사용 가이드 & 추천 도구"):
    st.markdown("""
| 단계 | 도구 | 링크 |
|------|------|------|
| 1️⃣ 스크립트 | 이 앱 | — |
| 2️⃣ AI 음성 | 타입캐스트 | [typecast.ai](https://typecast.ai) |
| 3️⃣ 이미지 | Pexels | [pexels.com](https://pexels.com) |
| 4️⃣ 편집 | CapCut | [capcut.com](https://capcut.com) |
| 5️⃣ BGM | YT 오디오 라이브러리 | [studio.youtube.com](https://studio.youtube.com) |
| 6️⃣ 업로드 | YouTube Studio | — |
""")
